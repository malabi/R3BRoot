...
Double_t GetERec(Double_t Ep, Double_t Th, Double_t Phi, Double_t Ebeam){
  ...
  //  pb = sqrt((12*amumev*2*Ebeam*1000) + pow(Ebeam*1000,2));  // in MeV for 12C
  pb = sqrt((47.95252277*amumev*2*Ebeam*1000) + pow(Ebeam*1000,2));  // in MeV  for 48Ca
	  
  p3l = sqrt(pow((Ep*1000+1.007825*amumev),2) - pow((1.007825*amumev+Ex),2)); // in MeV
  ...
  erec = sqrt(pow(prec,2) + pow(12*amumev,2)) /1000.;  // total energy (GeV)
  
  //trec = erec - 12*amumev/1000;  // kinetic energy (GeV)
  trec = erec - 47.95252277*amumev/1000;  // kinetic energy (GeV)
...
Float_t GetERec(Float_t Ep3, Float_t Ep4, Float_t Th3, Float_t Phi3, Float_t Th4, Float_t Phi4, Float_t Ebeam){
  ...
  //  pb = sqrt((12*amugev*2*Ebeam) + pow(Ebeam,2)); // for 12C
  pb = sqrt((47.95252277*amugev*2*Ebeam) + pow(Ebeam,2));  // for 48Ca
  ...
  //erec = sqrt(pow(prec,2) + pow(11.0093054*amugev,2)); // 12C -> B11
  erec = sqrt(pow(prec,2) + pow(46.9616616*amugev,2)); // 48Ca -> 47K
  
  //trec = erec - 11.0093054*amugev;  // 12C -> B11
...
Float_t GetThetaCM(Float_t th3l){   
  ...
  //Float_t m[5] = {0.,47.9525227,1.007825,1.007825,46.9616616};   // For elastic ??
  Float_t m[5] = {0.,47.9525227,1.007825,1.007825,47.9525227};   // For elastic ??
  ...
  Float_t a,b,c,d;
  Float_t amugev = 931.5016/1000.;
  //Float_t ebeam = 4.8;  // for 12C
  Float_t ebeam = 19.2;  // for 48Ca
...
void analysEvents(Int_t analType=1, const char* input="../../sim.root", const char* output="ana.root") {
  //
  ...
  TTree* T = (TTree*)file1->Get("evt");  // cbmsim is the name of the Tree
  ...
  TH1F *QV_fin = new TH1F("Qvalue_fin_fin","Qvalue reconstructed [GeV]",1000,-0.1,0.1);
  ...
  TH1F *YtrackDet = new TH1F("YtrackDet","G ",1001,0.,10.);
  ...
  TH1F *Theta_rad = new TH1F("Theta_rad","Reconstructed from hit strips",2000,0.08,1.9);
  TH1F *Theta = new TH1F("Theta","Reconstructed from hit strips",4000,5.,105.);
  ...
  TH2F *StripNStripP = new TH2F("Strip-n_Strip-p","G ",2101,0,2100, 2101,0,2100);
  ...
  TH1F *Theta_rec = new TH1F("Theta_rec","Reconstructed from track",4000,5.,105.);
  ...
  //For elastic case:
  TH1F *Theta_rec_rad = new TH1F("Theta_rec_rad","Reconstructed from track",2000,0.08,1.9);
  ...
  TH1F *EDiff = new TH1F("Qvalue","Energy (MeV)",2000,-20,20);
  TH1F *EDiff2 = new TH1F("Qvalue","Energy (MeV) - theta<75deg ",2000,-20,20);
  //TH1F *EDiff = new TH1F("Qvalue","Energy (MeV)",2000,-20.,20.);
  ...
  TH1F *ERecons = new TH1F("ERecons","Reconstructed Excitation energy",1000,-5,5);
...
*/
  ...
  Double_t Ebeam = 19.2;  // GeV (= 400MeV/u x48)
...
// For Si tracker:
  ...
  Double_t StripPitch1= 0.00385 + 0.0012 + 0.000229 ; // = 52.77 um  = strip pitch (38.5um) + interstrip (~14 um -to be checked)
  ...
  //Double_t Length2  = 33.83875 ; // cm
	...
	Double_t Length2  = 33.83875 + 2*SensorGap2 - (Guard2 + Guard2-0.002) ; // cm  ; length of active area (strips)
  //Double_t WidthMax2  = 10.4 ; // cm
  //Double_t WidthMax2  = 10.80295 ; // cm
	Double_t WidthMax2  = 2048.*(50.5e-4)/cos(8.116*pi/180) ; // cm ; active area (strips) max width
  //Double_t WidthMin2  = 1.3 ; // cm
  //Double_t WidthMin2  = 1.1406 ; // cm
	Double_t WidthMin2  = 172.*50.5e-4/cos(8.116*pi/180) ; // cm ; active area (strips) min width
  ...
  Double_t Rmin2=2.30657; // cm  
  //Double_t AngRangeMin2=5.3; // deg    
  Double_t AngRangeMin2=5.537946491; // 5.3;// deg
  //Double_t AngRangeMin2=5.306; // 5.3;// deg
		  ...
		  // then transform back in Lab frame:
			   ...
			   if(StripA_Id>=0 && StripB_Id>=0)
			       ...
			       if(ELoss[j]>0.00004)  //40 keV
				 {
			       ...
			       Theta_rad->Fill(Theta_track[j]*pi/180.);
...
//  -0.07 to have 172 strips at the narrow end of the ladder !!! // + 0.03; // shift along z axis (z lab coordinate of the center of the detector after inverse transformation, 0.03 is an extra shift thought to be coinciding with the middle line of a strip: to be checked !! ).
		  ...
		  // find intersection of the 2 hit strips:
			  ...
			  //X_intersect= X_track_det[j];  
			  Y_intersect= Y_track_det[j];
			  //cout << "Y_intersect=" << Y_intersect << endl;  
			  Z_intersect= SlopA*X_intersect+OffsetA;  
			  //Z_intersect= Z_track_det[j];
			  ...
			  // fill histograms
			   if(StripA_Id>=0 && StripB_Id>=0)
			       ...
			       if(StripB_Id>NbStrip2)
				 {
			       ...
			       Theta->Fill(Theta_track[j]);
			       Phi->Fill(Phi_track[j]);
			       ELossMiddl->Fill(ELoss[j]*1000); // MeV
			       
			       StripNStripP->Fill(StripA_Id, StripB_Id);
			       if(StripB_Id==1)cout << "StripA_Id=" << StripA_Id << endl;
			  ...
			  // fill histograms
			   if(StripA_Id>=0 && StripB_Id>=0)
			       ...
			       if(StripB_Id>NbStrip3)
				 {
			       ...
			       ELossOuter->Fill(ELoss[j]*1000);  // MeV
...
/////////////////////////////////////////////////////////////////
// For p,2p one expect at least  TrackMult=4 (2 hits in 2 Layers)
//
//
	...
	if( TrackMult==4 )   
	  ...
	  Double_t Xa[4]  ={0,0,0,0};
	  Double_t Ya[4]  ={0,0,0,0};
	  Double_t Za[4]  ={0,0,0,0};
	  Double_t Phia[4]={0,0,0,0};
	  Double_t Xb[4]  ={0,0,0,0};
	  Double_t Yb[4]  ={0,0,0,0};
	  Double_t Zb[4]  ={0,0,0,0};
	  Double_t Phib[4]={0,0,0,0};
	  ...
	  if(x==2 && y==2)  // inner and middle layer
	      ...
	      if( fabs((Phia[0]-Phib[0]))<1 )
		{
		  ...
		  Phi1Phi2->Fill(phi1*180/pi,phi2*180/pi);
   	          ...
   	          Theta_rec->Fill(theta1*180./pi);
   	          Theta_rec->Fill(theta2*180./pi);
	          Phi_rec->Fill(Phi1*180./pi);
	          Phi_rec->Fill(Phi2*180./pi);
		  ...
		  if(abs(theta1*TMath::RadToDeg() - Theta_init[0]) < abs(theta1*TMath::RadToDeg()- Theta_init[1]) )
		{
		...
		}else
		 {
		   ...
		   Theta_diff->Fill( (theta2*TMath::RadToDeg())- Theta_init[0]);
	        ...
	        //cout << "theta1 = " << theta1*180./pi << " Theta_init 0:" <<  Theta_init[0]<< endl; 
		//cout << "phi1 = " << phi1*180/pi << endl; 
		//cout << "theta2 = " << theta2*180./pi << " Theta_init 1:" <<  Theta_init[1] << endl; 
		//cout << "phi2 = " << phi2*180/pi << endl; 


		        //ERecoil= GetERec(Ep1, Ep2, theta1, phi1, theta2, phi2, Ebeam);
			//cout << "ERecoil [GeV] = " << ERecoil << endl;
		        //ERec->Fill(ERecoil);
 
	  	        //Qvalue_fin = Ebeam - (Ep1_fin+Ep2_fin) - ERecoil;    //in GeV  for 48Ca at 400MeV/u and  Califa with infinite  dimension) 
		        //QV_fin->Fill(Qvalue_fin);
...
if(TrackMult==1){   // particle stop in first layer hit
	  ...
	  TotalE[j]=0.;
		...
		{     
		  ...
		  if(j==(TrackMult-1)){  // last tracker hit
		    ...
		    CsIE = Eng-TotalE[j];   // For an ideal calorimeter
		    CsIER = gRandom->Gaus(CsIE,(CsIE*0.01)) ; // 1%
		    //CsIER = CsIE ;  // 0%
		    TotE = CsIER + TotalE[j];  // TotE = total energy of the proton
 ...
 if( TrackMult==2 && (DetCopyID[0] != DetCopyID[1])){      // !! works only for 2 layers of Si
	    ...
	    if(Zdiff != 0.){
	      ...
	      Theta_rec->Fill(Theta_trk);
	      Phi_rec->Fill(Phi_trk);
		...
		*/
		...
		//Erec=GetERec(TotE, Theta_trk*TMath::DegToRad(), Phi_trk*TMath::DegToRad(), 4.8); // for 12C at 400 MeV/u
		Erec=GetERec(TotE, Theta_trk*TMath::DegToRad(), Phi_trk*TMath::DegToRad(), 19.2);  // for 48Ca at 400 MeV/u
		//Erec=GetERec(Eng, Angle*TMath::DegToRad(), 4.8);

		//cout << "TotE=" << TotE << " Eng=" << Eng << " Erec=" << Erec << " Angle=" << Angle << endl;
	    
		//Qvalue = 4.8 - Eng - Erec;    // in GeV
		Qvalue = 19.2 - Eng - Erec;    // in GeV
		//cout << "Qvalue=" << Qvalue << "GeV" << endl;
		ERecons->Fill(Erec*1000.); // in MeV
		EDiff->Fill(Qvalue*1000.); // in MeV
		if(Theta_trk<75)EDiff2->Fill(Qvalue*1000.); // in MeV
