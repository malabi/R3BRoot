...
Float_t GetERec_p2p(Float_t Ep3, Float_t Ep4, Float_t Th3, Float_t Phi3, Float_t Th4, Float_t Phi4, Float_t Ebeam, Float_t Mbeam, Float_t Mrecoil, Float_t Mp);
...
void RecResults(const char* output) {
  ...
  gStyle->SetOptFit(0);
	...
	//SETTINGS 
	Double_t M_p; // Mass proton, set later on to get same value than MC transport code
	// For 48Ca(p,2p): (Q_value= -15.8)
	//Double_t M_beam= 47.95252277; // in uma for 48Ca -> 47K
	//Double_t M_recoil= 46.9616616; // in uma  for 48Ca -> 47K
	//Double_t E_beam= 19.2; // in GeV for 48Ca (400MeV/A)-> 47K
	// For 12C(p,2p): (Q_value= -15.957)
	Double_t M_beam= 12.; // in uma for 12C -> 11B
	Double_t M_recoil= 11.0093054; // in uma  for 12C -> 11B
	//Double_t E_beam= 4.8; // in GeV for 12C (400MeV/A)-> 11B
	Double_t E_beam= 6; // in GeV for 12C (500MeV/A)-> 11B
	...
	*/
	...
	sprintf(title0,"%s","startrackerAnaDigit.root");  // File with only detector information 	
 	sprintf(title1,"%s","../../r3bsim.root");         // File with all information
	...
	TH1F *Theta = new TH1F("Theta","Reconstructed from hit strips w/r to (0,0,0) ",180,0.,180.);  // assuming emission position at (0,0,0)
	TH1F *Phi = new TH1F("Phi","Reconstructed from hit strips w/r to (0,0,0)",360,0.,360.);      // assuming emission position at (0,0,0)

  	TH1F *Theta_rec = new TH1F("Theta_rec","Reconstructed from track",4000,5.,105.);  // when 1 incoming proton only
  	TH1F *Phi_rec = new TH1F("Phi_rec","Reconstructed from tracks",360,0.,360.);
	TH1F *Theta_diff = new TH1F("Theta_diff","Difference between Theta emitted and Theta reconstructed from tracks",4000,-40.,40.);
	...
	//For p2p case:
	TH1F *AngTheta1 = new TH1F("AngTheta1","Reconstructed from hit strips w/r to (0,0,0) ",180,0.,180.);
	TH1F *AngTheta2 = new TH1F("AngTheta2","econstructed from hit strips w/r to (0,0,0)",180,0.,180.);
	TH2F *Theta1Theta2 = new TH2F("Theta1Theta2","G ",180,0.,180., 180,0.,180.);
	TH1F *AngPhi1 = new TH1F("AngPhi1","Reconstructed from hit strips w/r to (0,0,0)",180,0.,360.);
	TH1F *AngPhi2 = new TH1F("AngPhi2","Reconstructed from hit strips w/r to (0,0,0) ",180,0.,360.);
	...
	TH1F *Trecoil = new TH1F("Trecoil"," Kinetic energy of recoil (GeV)",1000.,-50.,50);
	TH1F *Qval = new TH1F("Qvalue","Recontructed Q-value in MeV",1000,-50,50);

	//Input trees:
	TTree* TStarTrackDigit = (TTree*)file0->Get("evt");  // To access information as real detector
	TTree* TStarTracker = (TTree*)file1->Get("evt");     // To access Simulation input information 
	...
	// Information of emitted particle (only known in the simulation)
	...
	*/
	...
	Double_t *Theta_init;    // polar angle of primaries at vertex
	Double_t *Mass_part;          // Mass of primaries at vertex (GeV/c2)
	Double_t *Ekin_init;     // Kinetic of primaries at vertex


	// For detected particle:	
	// For Si tracker:
	Int_t TrackMult=0;
	...
	//
	// Si Geometrical parameters :
	//
	
	// Inner layer
	//Double_t Length1  = 19.03 ; // cm
	Double_t Length1  = 21.794 + 0.015; // cm  ( +150um to count for the gap between sensor)
	...
	Double_t StripPitch1= 0.00385 + 0.0012 + 0.0001 + 0.000129; // = 52.79 um
	...
	Double_t ZRotAngle1=30.;
	Xlab1=-((Length1/2)*sin(InclAng1*pi/180.)+ Rmin1)*sin(ZRotAngle1*pi/180.);
	Ylab1=-((Length1/2)*sin(InclAng1*pi/180.)+ Rmin1)*cos(ZRotAngle1*pi/180.);
	...
	//Double_t Guard2  = 0.154 ; // cm  ; = distance edge detector to strip area !! to be checked !
	Double_t Guard2  = 0.1589 ; // cm  ; = distance edge detector to strip area !! to be checked !
	...
	Double_t AngTrap2= atan((WidthMax2/2 -WidthMin2/2)/Length2); // ~0.1416 ; 
	...
	Double_t WidthMax3  = WidthMax2;
	//Double_t WidthMin3  = 1.1406 ; // cm
	Double_t WidthMin3  = WidthMin2; // cm
	Double_t StripPitch3= 0.00385 + 0.0012; // = 50.5 um
	Double_t InclAng3=32.155; // deg    
	Double_t Rmin3= 3.03196; //2.95;    // cm
  ...
  */
   ...
   // Middle layer:
	...
	Double_t M_INV_Mid[12][4][4]=
	  {
	    {
	      { 0.965925826,	0.219119161,	0.137746476,	-3.144883119},  // Matrice 1 row 0
	      {-0.258819045,	0.81776384,	0.514076846,	-11.73686358},  // Matrice 1 row 1
	      { 0,             -0.532211513,    0.846611425,     18.96166758},  // Matrice 1 row 2
	      { 0,            0,             0,            1}   // Matrice 1 row 3
	    },
	    
	    {
	      {0.707106781,	0.59864468,	0.37633037,	-8.591980465},  // Matrice 2 row 0
	      {-0.707106781,	0.59864468,	0.37633037,	-8.591980465},  // Matrice 2 row 1
	      {          0,  -0.532211513,	0.846611425,	18.96166758},  // Matrice 2 row 2
	      {          0,             0,             0,             1}   // Matrice 2 row 3
	    },
	    
	    {
	      { 0.258819045,	0.81776384,	0.514076846,	-11.73686358},  // Matrice 3 row 0
	      {-0.965925826,	0.219119161,	0.137746476,	-3.144883119},  // Matrice 3 row 1
	      {           0,  -0.532211513,	0.846611425,	18.96166758},  // Matrice 3 row 2
	      {           0,             0,            0,             1}   // Matrice 3 row 3
	    },
	    
	    {
	      { -0.258819045,	0.81776384,	0.514076846,	-11.73686358},  // Matrice 4 row 0
	      {  -0.965925826,	-0.219119161,	-0.137746476,	3.144883119},  // Matrice 4 row 1
	      {         0.,   -0.532211513,	0.846611425,	18.96166758},  // Matrice 4 row 2
	      {         0.,              0,            0,             1}   // Matrice 4 row 3
	    },
	    
	    {
	      { -0.707106781,	0.59864468,	0.37633037,	-8.591980465},  // Matrice 5 row 0
	      {-0.707106781,	-0.59864468,	-0.37633037,	8.591980465},  // Matrice 5 row 1
	      {           0,  -0.532211513,	0.846611425,	18.96166758},  // Matrice 5 row 2
	      {           0,             0,            0,             1}   // Matrice 5 row 3
	    },
	    
	    {
	      { -0.965925826,	0.219119161,	0.137746476,	-3.144883119},  // Matrice 6 row 0
	      { -0.258819045,	-0.81776384,	-0.514076846,	11.73686358},  // Matrice 6 row 1
	      {            0,  -0.532211513,	 0.846611425,	18.96166758},  // Matrice 6 row 2
	      {            0,             0,             0,             1}   // Matrice 6 row 3
	    },
	    
	    {
	      {-0.965925826,	-0.219119161,	-0.137746476,	3.144883119},  // Matrice 7 row 0
	      { 0.258819045,	-0.81776384,	-0.514076846,	11.73686358},  // Matrice 7 row 1
	      { 0,              -0.532211513,	 0.846611425,	18.96166758},  // Matrice 7 row 2
	      { 0,            0,                 0,             1}   // Matrice 7 row 3
	    },
	    
	    {
	      {-0.707106781,	-0.59864468,	-0.37633037,	8.591980465},  // Matrice 8 row 0
	      { 0.707106781,	-0.59864468,	-0.37633037,	8.591980465},  // Matrice 8 row 1
	      {           0,  -0.532211513,	 0.846611425,	18.96166758},  // Matrice 8 row 2
	      {           0,             0,             0,              1}   // Matrice 8 row 3
	    },
	    
	    {
	      { -0.258819045,	-0.81776384,	-0.514076846,	11.73686358},  // Matrice 9 row 0
	      { 0.965925826,	-0.219119161,	-0.137746476,	3.144883119},  // Matrice 9 row 1
	      {           0,  -0.532211513,	0.846611425,	18.96166758},  // Matrice 9 row 2
	      {           0,             0,            0,             1}   // Matrice 9 row 3
	    },
	    
	    {
	      { 0.258819045,	-0.81776384,	-0.514076846,	11.73686358},  // Matrice 10 row 0
	      { 0.965925826,	0.219119161,	0.137746476,	-3.144883119},  // Matrice 10 row 1
	      {         0.,   -0.532211513,	0.846611425,	18.96166758},  // Matrice 10 row 2
	      {         0.,              0,            0,             1}   // Matrice 10 row 3
	    },
	    
	    {
	      { 0.707106781,	-0.59864468,	-0.37633037,	8.591980465},  // Matrice 11 row 0
	      { 0.707106781,	0.59864468,	0.37633037,	-8.591980465},  // Matrice 11 row 1
	      {           0,  -0.532211513,	0.846611425,	18.96166758},  // Matrice 11 row 2
	      {           0,             0,            0,              1}    // Matrice 11 row 3
	    },
	    
	    {
	      { 0.965925826,	-0.219119161,	-0.137746476,	3.144883119},  // Matrice 12 row 0
	      { 0.258819045,	0.81776384,	0.514076846,	-11.73894487},  // Matrice 12 row 1
	      {           0,  -0.532211513,	0.846611425,	18.96166758},  // Matrice 12 row 2
	      {           0,             0,             0,              1}    // Matrice 12 row 3
	    }
	  };
...
/*	Double_t M_INV_Out[12][4][4]=
	  {
	    {
	      {0.965925826,	0.219119161,	0.137746476,	-3.175202367},  // Matrice 1 row 0
	      {-0.258819045,	0.81776384,	0.514076846,	-11.85001656},  // Matrice 1 row 1
	      { 0,             -0.532211513,	0.846611425,	 20.14798005},  // Matrice 1 row 2
	      {0,            0,                 0,               1}   // Matrice 1 row 3
	    },
	    
	    {
	      {0.707106781,	0.59864468,	0.37633037,	-8.674814192},  // Matrice 2 row 0
	      {-0.707106781,	0.59864468,	0.37633037,	-8.674814192},  // Matrice 2 row 1
	      {          0,  -0.532211513,	0.846611425,	 20.14798005},  // Matrice 2 row 2
	      {          0,             0,             0,             1}   // Matrice 2 row 3
	    },
	    
	    {
	      { 0.258819045,	0.81776384,	0.514076846,	-11.85001656},  // Matrice 3 row 0
	      {-0.965925826,	0.219119161,	0.137746476,	-3.175202367},  // Matrice 3 row 1
	      {           0,  -0.532211513,	0.846611425,	20.14798005},  // Matrice 3 row 2
	      {           0,             0,            0,             1}   // Matrice 3 row 3
	    },
	    
	    {
	      { -0.258819045,	0.81776384,	0.514076846,	-11.85001656},  // Matrice 4 row 0
	      { -0.965925826,	-0.219119161,	-0.137746476,	3.175202367},  // Matrice 4 row 1
	      {         0.,   -0.532211513,	0.846611425,	 20.14798005},  // Matrice 4 row 2
	      {         0.,              0,            0.,               1}   // Matrice 4 row 3
	    },
	    
	    {
	      {-0.707106781,	0.59864468,	0.37633037,	-8.674814192},  // Matrice 5 row 0
	      {-0.707106781,	-0.59864468,	-0.37633037,	8.674814192},  // Matrice 5 row 1
	      {           0,  -0.532211513,	0.846611425,	20.14798005},  // Matrice 5 row 2
	      {           0,             0,            0,              1}   // Matrice 5 row 3
	    },
	    
	    {
	      { -0.965925826,	0.219119161,	0.137746476,	-3.175202367},  // Matrice 6 row 0
	      {  -0.258819045,	-0.81776384,	-0.514076846,	11.85001656},  // Matrice 6 row 1
	      {            0,  -0.532211513,	0.846611425,	20.14798005},  // Matrice 6 row 2
	      {            0,             0,            0,             1}   // Matrice 6 row 3
	    },
	    
	    {
	      {-0.965925826,	-0.219119161,	-0.137746476,	3.175202367},  // Matrice 7 row 0
	      { 0.258819045,	-0.81776384,	-0.514076846,	11.85001656},  // Matrice 7 row 1
	      { 0, -0.532211513,	 0.846611425,	20.14798005},  // Matrice 7 row 2
	      { 0,            0,                 0,             1}   // Matrice 7 row 3
	    },
	    
	    {
	      {-0.707106781,	-0.59864468,	-0.37633037,	8.674814192},  // Matrice 8 row 0
	      { 0.707106781,	-0.59864468,	-0.37633037,	8.674814192},  // Matrice 8 row 1
	      {           0,  -0.532211513,	0.846611425,	20.14798005},  // Matrice 8 row 2
	      {           0,             0,             0,            1}   // Matrice 8 row 3
	    },
	    
	    {
	      { -0.258819045,	-0.81776384,	-0.514076846,	11.85001656},  // Matrice 9 row 0
	      { 0.965925826,	-0.219119161,	-0.137746476,	3.175202367},  // Matrice 9 row 1
	      {           0,  -0.532211513,	0.846611425,	20.14798005},  // Matrice 9 row 2
	      {           0,             0,            0,             1}   // Matrice 9 row 3
	    },
	    
	    {
	      { 0.258819045,	-0.81776384,	-0.514076846,	11.85001656},  // Matrice 10 row 0
	      { 0.965925826,	0.219119161,	0.137746476,	-3.175202367},  // Matrice 10 row 1
	      {         0.,   -0.532211513,	0.846611425,	20.14798005},  // Matrice 10 row 2
	      {         0.,              0,            0,             1}   // Matrice 10 row 3
	    },
	    
	    {
	      { 0.707106781,	-0.59864468,	-0.37633037,	8.674814192},  // Matrice 11 row 0
	      { 0.707106781,	0.59864468,	0.37633037,	-8.674814192},  // Matrice 11 row 1
	      {           0,  -0.532211513,	0.846611425,	20.14798005},  // Matrice 11 row 2
	      {           0,             0,            0,              1}    // Matrice 11 row 3
	    },
	    
	    {
	      { 0.965925826,	-0.219119161,	-0.137746476,	3.17417245},  // Matrice 12 row 0
	      { 0.258819045,	0.81776384,	0.514076846,	-11.85001656},  // Matrice 12 row 1
	      {           0,  -0.532211513,	0.846611425,	20.14798005},  // Matrice 12 row 2
	      {           0,             0,             0,              1}    // Matrice 12 row 3
	    }
	  };
*/
	...
	// Get number of simulated events in the trees ( it is the same in the 2 input files/trees)
	...
	Int_t nb = 0;  // for first root input file
	Int_t nb2 = 0; // for 2nd root input file
	...
	//for(Int_t i=0;i<5;i++){
	    ...
	    //if one input tree:     
	    nb += TStarTrackDigit->GetEvent(i);  // get event in first tree
	    nb2 += TStarTracker->GetEvent(i);    // get event in 2nd tree
	    ...
	    //
	    // Simulation Input information for comparison checks
	    //	       
	    PartMult= brMCStackPDG->GetNdata(); // particle multiplicity: 1 for (p,p); 2 for (p,2p)
	    ...
	    Theta_init=new Double_t[PartMult];   // polar angle of emitted proton at vertex 
	    Mass_part=new Double_t[PartMult];   // polar angle of emitted proton at vertex 
	    Ekin_init=new Double_t[PartMult];   // polar angle of emitted proton at vertex 
		...
		Mass_part[j]=brMCStackMass->GetValue(j,0,true);
		//cout << "Mass_part=" << Mass_part[j] << endl;

	        if(Mother_part[j]==-1 && PDG_part[j]==2212){  // PDG for proton= 2212

		 M_p= Mass_part[j]; // Set proton mass as define in the simulation
	         Theta_init[j]=acos(PZ_part[j]/sqrt(pow(PX_part[j],2)+pow(PY_part[j],2)+pow(PZ_part[j],2)))*TMath::RadToDeg();   // degrees;
	         //cout << "Theta_init="<< Theta_init[j] << " for j=" << j << endl;
		 Ekin_init[j]=GetEnergy(PX_part[j],PY_part[j],PZ_part[j],Mass_part[j]);
	         //cout << "Ekin_init="<< Ekin_init[j] << " for j=" << j << endl;
	        }
	    ...
	    //
	    // Information collected in Si Tracker:
	    //
	    //TrackMult=branchTrackX->GetNdata();
	    //TrackMult=branchStripfrt->GetNdata();
	    TotalTrackMult= branchDigitELoss->GetNdata();  // (Multiplicity= 1 means 1 front OR 1 back strip hit)  <=> nb of strips with hit(s)

	    Chip=new Int_t[TotalTrackMult];     // Asic id
	    Side=new Int_t[TotalTrackMult];     // Side id
	    Strip=new Int_t[TotalTrackMult];     // Strip id
		...
		if(Side[j]==0)
		  ...
		  if(j==0)
		    ...
		    {
		      Stripfrt[int(j/2)]= Chip[j]*128 + (Strip[j]);
		      //cout << "Side= " <<  Side[j] << " j=" << j << endl;
		...
		if(Side[j]==1)    // This assume the first strip hit is the front strip : True in simulation but not guarranty in real data
		  {
		    ...
		    if(j%2 !=0 )
		      {
			Stripbck[int((j-Side[j])/2)]= Chip[j]*128 + (Strip[j]);
			...
			if(j==0)
			  {
			    Stripbck[j]= Chip[j]*128 + (Strip[j]);
			  }else
			  {
			    Stripbck[j-Side[j]]= Chip[j]*128 + (Strip[j]);
			  }
		...
		if(Stripfrt[j]==-1 && Stripbck[j]==-1)
		  {
		    cout << "  The particle hit the Si detector but outside the area covered by the strips !!" << endl;
		    cout << "  Stripfrt[j]==-1 && Stripbck[j]==-1 !!" << endl;
		  }else
		  {
			...
			// -0.07 to have 172 strips at the narrow end of the ladder //+ 0.03; // shift along z axis (-1.35855= z lab coordinate of the center of 
			...
			OffsetA= (-Length1/2 + ShiftalongZ) - SlopA*(WidthMax1/2 - (StepX1/2)*(2*Stripfrt[j]+1));
		  ...
		  // then transform in Lab frame:
			   ...
			   Z_track[j]=   X_intersect*M_INV_Inner[DetCopyID[j]-1][2][0] 
			    ...
			    if(Stripfrt[j]>NbStrip1-1)cout << "StripA_Id>NbStrip1 in event #"<< i << endl;
			    if(Stripbck[j]>NbStrip1-1)cout << "StripB_Id>NbStrip1 in event #"<< i << endl;
			   ...
			   if(Stripfrt[j]>=0 & Stripbck[j]>=0)
			     {
			       StripA_Inner->Fill(Stripfrt[j]+1);
			       StripB_Inner->Fill(Stripbck[j]+1);
			       if(ELoss[j]>0.00004)  //40 keV
				 {
				   StripA_Inner40->Fill(Stripfrt[j]+1);
				   StripB_Inner40->Fill(Stripbck[j]+1);
				 }
			       ...
			       StripNStripP->Fill(Stripfrt[j]+1, Stripbck[j]+1);
			...
			Y_intersect= 5.789;   // Constant determined by the way the transformation matrices have been built (see YTrack_det analysLampShadeSpec1Tree.C) 
			...
			if(Stripfrt[j]>NbStrip2-1)
			  {
			    cout << "StripA_Id>NbStrip2-1 in event #"<< i << endl;
			  }
			if(Stripbck[j]>NbStrip2-1)
			  {
			    cout << "StripB_Id>NbStrip2-1 in event #"<< i << endl;
			  }
			...
			if(Stripfrt[j]>=0 & Stripbck[j]>=0)
			  {
			    StripA_Middl->Fill(Stripfrt[j]+1);
			    StripB_Middl->Fill(Stripbck[j]+1);
			    if(ELoss[j]>0.00004) // 40 keV
			      {
				StripA_Middl40->Fill(Stripfrt[j]+1);
				StripB_Middl40->Fill(Stripbck[j]+1);
			      }
			    Xtrack->Fill(X_track[j]);
			    Ytrack->Fill(Y_track[j]);
			    Ztrack->Fill(Z_track[j]);
			...
			ShiftalongZ= ( Xlab3*M_Out[0][2][0] 
				       + Ylab3*M_Out[0][2][1] 
				       + Zlab3*M_Out[0][2][2] 
				       +     1*M_Out[0][2][3])-0.07; // -0.07 to have 172 strips at the narrow end of the ladder //+ 0.03; // shift along z axis (z lab coordinate of the center of the detector after inverse tr			ansformation, 0.03 is an extra shift thought to be coinciding with the middle line of a strip: to be checked !! ).
			...
			if(Stripfrt[j]>NbStrip3-1)
			  {
			    cout << "StripA_Id>NbStrip3-1 in event #"<< i << endl;
			  }
			if(Stripbck[j]>NbStrip3-1)
			  {
			    cout << "StripB_Id>NbStrip3-1 in event #"<< i << endl;
			  }
			...
			if(Stripfrt[j]>=0 & Stripbck[j]>=0)
			  {
			    ...
			    StripA_Outer->Fill(Stripfrt[j]+1);
			    StripB_Outer->Fill(Stripbck[j]+1);
			    
			    if(ELoss[j]>0.00004)   // 40 keV
			      {
				StripA_Outer40->Fill(Stripfrt[j]+1);
				StripB_Outer40->Fill(Stripbck[j]+1);
			      }
			    Xtrack->Fill(X_track[j]);
			    Ytrack->Fill(Y_track[j]);
			    Ztrack->Fill(Z_track[j]);
	    ...
	    ///////////////////////////////////////////////////////////////////////////
	    //
	    // Reconstruction for 1 proton only:
	    //
	    if(TrackMult>=2)
	    {
		...
		if(x>=1 && y>=1)  // At least inner and intermediate layer are hit once each
		{
		    theta1_rec=0.;
		    phi_rec=0.;
		    Ep_rec=0.;
			...
			phi_rec= GetPhiTrk(Xb[0]-Xa[0], Yb[0]-Ya[0]);                   // one calculates the azimuthal angle form the position in inner and intermediate layer
			theta_rec= GetThetaTrk(Xb[0]-Xa[0], Yb[0]-Ya[0], Zb[0]-Za[0]);  // one calculates the polar angle form the position in inner and intermediate layer
			Ep_rec=Epa[0]+Epb[0];

			Theta_rec->Fill(theta_rec*180/pi);
			Phi_rec->Fill(phi_rec*180/pi);
		...
		Double_t Ep1, Ep2, E_calif1, E_calif2;
		Double_t Erec=0.;
		Double_t Qvalue=0.;
		...
		part_a=new Int_t[TrackMult_Inner40];
		part_b=new Int_t[TrackMult_Middl40];
		part_c=new Int_t[TrackMult_Outer40];
		*////////////////////////////////////////
		...
		if(x>=2 && y>=2)  // At least inner and intermediate layer are hit (= better case than inner and outer layers because of straggling in intermediate layer 
		  {
			...
			// We compare with initial angle to make sure the index correspond to first or second proton
			// Note: not possible with real data but with real data one would use other criters and Califa data as well 
			// See general tracking
  		        if(abs(theta1*TMath::RadToDeg() - Theta_init[0]) < abs(theta1*TMath::RadToDeg()- Theta_init[1]) )
			  { 
		          Theta_diff->Fill( (theta1*TMath::RadToDeg())- Theta_init[0]);
		          Theta_diff->Fill( (theta2*TMath::RadToDeg())- Theta_init[1]);
			  ...
			  E_calif1=Ekin_init[0]-Ep1; // remaining energy of the proton for ideal califa and no chamber
			  E_calif2=Ekin_init[1]-Ep2; // remaining energy of the proton for ideal califa and no chamber


		        }else
		        {
		          Theta_diff->Fill( (theta1*TMath::RadToDeg())- Theta_init[1]);
		          Theta_diff->Fill( (theta2*TMath::RadToDeg())- Theta_init[0]);



			  Ep1=Epa[1]+Epb[1];
			  //Ep1=Epa[1]+Epb[1]+Epc[1];  // with 3rd layer
			  Ep2=Epa[0]+Epb[0];
			  //Ep2=Epa[0]+Epb[0]+Epc[0];  // with 3rd layer

			  E_calif1=Ekin_init[1]-Ep1; // remaining energy of the proton for ideal califa and no chamber
			  E_calif2=Ekin_init[0]-Ep2; // remaining energy of the proton for ideal califa and no chamber

		        }

			Ep1=gRandom->Gaus(Ep1,0.00007/2.35);  // 70 keV resolution for the 2 layers (<=>50 keV resolution in each layer)
			Ep2=gRandom->Gaus(Ep2,0.00007/2.35);  // 70 keV resolution for the 2 layers (<=>50 keV resolution in each layer)

			E_calif1=gRandom->Gaus(E_calif1,(E_calif1*0.01)/2.35); ; // remaining energy of the proton for 1% reso in califa and no chamber
			E_calif2=gRandom->Gaus(E_calif2,(E_calif2*0.01)/2.35); ; // remaining energy of the proton for 1% reso in califa and no chamber
			

			dEp1->Fill(Ep1*1000.); // Energy deposited by 1st proton in Si
			dEp2->Fill(Ep2*1000.); // Energy deposited by 2nt proton in Si

			Ep1= Ep1+E_calif1;  // reconstruct total energy of 1st proton
			Ep2= Ep2+E_calif2;  // reconstruct total energy of 2nd proton

			/*
			cout << "Ep1 tot reconstructed = " << Ep1                      << endl;
			cout << "Theta1 reconstructed = "  << theta1*TMath::RadToDeg() << endl;
			cout << "Phi1 reconstructed = "    << phi1*TMath::RadToDeg()   << endl;
			cout << "Ep2 tot reconstructed ="  << Ep2                      << endl;
			cout << "Theta2 reconstructed ="   << theta2*TMath::RadToDeg() << endl;
			cout << "Phi2 reconstructed ="     << phi2*TMath::RadToDeg()   << endl;
			*/

			// Kinetic energy of recoil
	   	        Erec = GetERec_p2p(Ep1, Ep2, theta1, phi1, theta2, phi2, E_beam, M_beam, M_recoil, M_p);  // Kinetic energy of recoil
			Trecoil->Fill(Erec);  // Kinetic energy of recoil

			Qvalue = (Erec + Ep1 + Ep2)-E_beam; 
		        Qval->Fill(Qvalue*1000); //in MeV
...
/* Delete only the pointer that have been filled up */
	...
	//delete [] part_c;
        ...
        delete [] TotalE;  // Energy loss for a same track 
	delete [] E_particle; // 
...
Float_t GetERec(Float_t Ep3, Float_t Ep4, Float_t Th3, Float_t Phi3, Float_t Th4, Float_t Phi4, Float_t Ebeam){
  ...
  //pb = sqrt((12*amugev*2*Ebeam) + pow(Ebeam,2));  // for 12C beam 
  pb = sqrt((47.95252277*amugev*2*Ebeam) + pow(Ebeam,2));  // for 48Ca beam
  ...
  //erec = sqrt(pow(prec,2) + pow(11.0093054*amugev,2));  //12C -> 11B
  erec = sqrt(pow(prec,2) + pow(46.9616616*amugev,2)); // 48Ca -> 47K
  
  //trec = erec - 11.0093054*amugev;  //12C -> 11B
  trec = erec - 46.9616616*amugev;   // 48Ca -> 47K
...
Float_t GetERec_p2p(Float_t Ep3, Float_t Ep4, Float_t Th3, Float_t Phi3, Float_t Th4, Float_t Phi4, Float_t Ebeam, Float_t Mbeam, Float_t Mrecoil, Float_t Mp){
  ...
  //pb = sqrt((12*amugev*2*Ebeam) + pow(Ebeam,2));  // for 12C beam 
  //pb = sqrt((47.95252277*amugev*2*Ebeam) + pow(Ebeam,2));  // for 48Ca beam
  pb = sqrt((Mbeam*amugev*2*Ebeam) + pow(Ebeam,2)); 
	  
  //p3l = sqrt(pow((Ep3+1.007825*amugev),2) - pow((1.007825*amugev+Ex),2));
  //p4l = sqrt(pow((Ep4+1.007825*amugev),2) - pow((1.007825*amugev+Ex),2));
  p3l = sqrt(pow((Ep3+Mp),2) - pow((Mp+Ex),2));
  p4l = sqrt(pow((Ep4+Mp),2) - pow((Mp+Ex),2));
  ...
  prec = sqrt(pow((p3x+p4x),2) + pow((p3y+p4y),2) +pow((pb-p3z-p4z),2)); // p reconstructed
  
  //erec = sqrt(pow(prec,2) + pow(11.0093054*amugev,2));  //12C -> 11B
  //erec = sqrt(pow(prec,2) + pow(46.9616616*amugev,2)); // 48Ca -> 47K
  erec = sqrt(pow(prec,2) + pow(Mrecoil*amugev,2)); // 48Ca -> 47K // E reconstructed
  
  //trec = erec - 11.0093054*amugev;  //12C -> 11B
  //trec = erec - 46.9616616*amugev;   // 48Ca -> 47K
  trec = erec - Mrecoil*amugev;   // 48Ca -> 47K
