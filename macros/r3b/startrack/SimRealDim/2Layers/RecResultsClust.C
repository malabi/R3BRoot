...
//     mode=2 no tracking ( device used only as position sensitive detector; i.e. when only one layer of Si is used for instance)         
//                 
//  -------------------------------------------------------------------------
...
void RecResultsClust(Int_t mode, const char* output) {
  ...
  gStyle->SetOptFit(0);
	...
	TH1F *Theta_rec = new TH1F("Theta_rec","Reconstructed from track",180,0.,180.);
	TH1F *Phi_rec = new TH1F("Phi_rec","Reconstructed from track",360,0.,360.);
	TH1F *dEp_rec = new TH1F("dEp_rec","nrj loss of proton 1,  in all tracker [MeV] ",250,0.,5.);   // nrj loss of proton 1,  in all tracker    
	...
	TH2F *StripNStripP = new TH2F("Strip-n_Strip-p","G ",2100,0.,2100., 2100,0.,2100.);  // N= front ; P= back
	...
	TH1F *dEp1 = new TH1F("dEp1","nrj loss of proton 1,  in all tracker [MeV] ",250,0.,5.);   // nrj loss of proton 1,  in all tracker    
	TH1F *dEp2 = new TH1F("dEp2","nrj loss of proton 2,  in all tracker [MeV]",250,0.,5.);   // nrj loss of proton 2, ain all tracker    
...
//	  {
	    ...
	    Double_t CountTrk2 = 0; // double since it is incremented by 0.5
//	  }
...
//	    Track=new Int_t[TrackMult];
	    ...
	    //EDepos1Cryst=new Double_t[TrackMult];
            ...
            theta=new Double_t[TrackMult];  		
            phi=new Double_t[TrackMult];    // using TrackMult_Middl40 here since we look for tracking between inner and intermediate layer.		
            Ep=new Double_t[TrackMult];     // using TrackMult_Middl40 here since we look for tracking between inner and intermediate layer.		
            part_a=new Int_t[TrackMult];
            part_b=new Int_t[TrackMult];
          //part_c=new Int_t[TrackMult];
	    ...
	    for(Int_t j=0; j<TrackMult; j++){
	      ...
	      //Xc[j]=Yc[j]=Zc[j]=Phic[j]=Epc[j]=0.; // for outer
		...
		if(DetCopyID[j]<7)
		  {
			   if(Stripfrt[j]>NbStrip1-1)cout << "StripA_Id=" <<  Stripfrt[j] <<  ">NbStrip1 in event #"<< i << endl;
			   if(Stripbck[j]>NbStrip1-1)cout << "StripB_Id= "<<  Stripbck[j] << ">NbStrip1 in event #"<< i << endl;
		  }
	      }
		...
		if(Stripfrt[j]==-1 && Stripbck[j]==-1)
		  ...
		  }else
		  {
			...
			ShiftalongZ= ( Xlab1*M_Inner[0][2][0] 
				       + Ylab1*M_Inner[0][2][1] 
				       + Zlab1*M_Inner[0][2][2] 
				       +     1*M_Inner[0][2][3])-0.07;  // -0.07 to have 172 strips at the narrow end of the ladder //+ 0.03; // shift along z axis (-1.35855= z lab coordinate of the center of 
		  ...
		  // then transform in Lab frame:
			...
			Y_intersect= 5.789;   // Constant determined by the way the transformation matrices have been built (see analysLampShadeSpec1Tree.C) 
			Z_intersect= SlopA*X_intersect+OffsetA;  
			...
			if(trk)
			  {
				...
				Ep[t]= Epa[ part_a[t] ]+ Epb[ part_b[t] ];  // energy loss in the two layers
				//Ep[t]= Epa[ part_a[t] ]+ Epb[ part_b[t] ]+ Epc[part_c[t] ];  // energy loss in the three layers

				Theta_rec->Fill(theta[t]*180/pi);
				Phi_rec->Fill(phi[t]*180/pi);
				dEp_rec->Fill(Ep[t]*1000.);
		...
		if( TrackMult>=4 )   // Minimum requirement to have 2 protons going through at least 2 layers
		  {
		    ...
		    */
		    ...
		    if(InLay!=TrkMult_Inner40) cout << "Attention x and TrckMult_inner40 should be identical !!! InLay: = "<< InLay << " TrkMult_inner40= " << TrkMult_Inner40 << endl;
		    if(InterLay!=TrkMult_Middl40) cout << "Attention x and TrckMult_inner40 should be identical !!! InterLa: = "<< InterLay << " TrkMult_Middl40= " << TrkMult_Middl40 << endl;
		    cout << "InLay=" << InLay << " TrkMult_Inner40=" << TrkMult_Inner40 << endl;
		    cout << "InterLay=" << InterLay << " TrkMult_Middl40=" << TrkMult_Middl40 << endl;
		    //if(OutLay!=TrkMult_Outer40) cout << "Attention x and TrckMult_inner40 should be identical !!! : OutLay= "<< OutLay << " TrkMult_outer40= " << TrkMult_Outer40 << endl;
				...
				if( fabs((Phia[ia]-Phib[ib]))<0.2 ) // = considered as same track (here we could maybe put more conditions to be more restrictive)
				  {
				    part_a[trk]=ia; 
				    part_b[trk]=ib;
				     trk++; // this is track #trk   (Will's correction )
				    //cout << "ia:" << ia << " ib:"<< ib << endl;
			...
			cout << "Number of track is higher than 1"  << endl;
			//cout << " "<< endl;
			Tracks->Fill(trk);
			...
			if(trk)
			  {
			    ...
			    if(trk>2)  // ie there is more than 2 tracks detected 
			      {
				//for (Int_t t=0;t<trk-1;t++) //  Will mods trk to trk-1 
				for (Int_t t=0;t<trk;t++) // 
				  {
				    //cout << "fabs(phi[t]-phi[t+1])=" << fabs(phi[t]-phi[t+1]) << " " << phi[t] << " " <<  phi[t+1] << endl;
				    if( fabs(phi[t]-phi[t+1])>(pi-1) && fabs(phi[t]-phi[t+1])<(pi+1) )
			...
			/////////////////// End of general tracking
			cout << "End of General tracking"  << endl;
		...
		// InvMass=->Fill(Inv_Mass);
	      ...
	      }// end of p2p track analysis (mode 1)
		
	cout << "End of p2p track analysis"  << endl;
