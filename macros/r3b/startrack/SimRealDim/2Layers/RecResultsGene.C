...
void RecResultsGene(const char* output) {
  ...
  gStyle->SetOptFit(0);
	...
	*/
	...
	Double_t StripPitch2= 0.00385 + 0.0012; // = 51.5 um    
...
// Transformation  matrices (ie transformation from Lab to det coord. system)  // Also to be stored in DB
  ...
  */
		...
		if(Stripfrt[j]==-1 && Stripbck[j]==-1)
		  ...
		  }else
		  {
			...
			ShiftalongZ= ( Xlab1*M_Inner[0][2][0] 
				       + Ylab1*M_Inner[0][2][1] 
				       + Zlab1*M_Inner[0][2][2] 
				       +     1*M_Inner[0][2][3])-0.07; // -0.07 to have 172 strips at the narrow end of the ladder 
                                                                       // + 0.03; // shift along z axis (-1.35855= z lab coordinate of the center of 
		  ...
		  // then transform in Lab frame:
			   ...
			   Z_track[j]=   X_intersect*M_INV_Inner[DetCopyID[j]-1][2][0] 
			    ...
			    if(Stripfrt[j]>NbStrip1-1)cout << "StripA_Id>NbStrip1-1 in event #"<< i << endl;
			    if(Stripbck[j]>NbStrip1-1)cout << "StripB_Id>NbStrip1-1 in event #"<< i << endl;
			...
			ShiftalongZ= ( Xlab2*M_Mid[0][2][0] 
				       + Ylab2*M_Mid[0][2][1] 
				       + Zlab2*M_Mid[0][2][2] 
				       +     1*M_Mid[0][2][3])-0.07; // -0.07 to have 172 strips at the narrow end of the ladder // + 0.03; // shift along z axis (z lab coordinate of the center of the detector after inverse transformation, 0.03 is an extra shift thought to be coinciding with the middle line of a strip: to be checked !! ).
			...
			Y_intersect= 5.789;   // Constant determined by the way the transformation matrices have been built (see Y_Track_det in analysLampShadeSpec1Tree.C) 
			 ...
			 if(trk)
			   {
				 ...
				 Ep[t]= Epa[ part_a[t] ]+ Epb[ part_b[t] ] ;  // energy loss in the three layer
  ...
  // Free memories by deleting dynamic pointers
